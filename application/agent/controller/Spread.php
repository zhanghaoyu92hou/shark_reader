...
class Spread extends Common{
    ...
    public function index(){
        if($this->request->isAjax()){
            ...
            $res = aSpread::getSpreadList($where, $pages);
    ...
    public function createLink(){
        if($this->request->isAjax()){
            ...
            aSpread::doneSpread($field);
        }else{
            $book_id = myRequest::getId('书籍','book_id');
            $book = aSpread::getById('Book',$book_id,'id,name,type');
            if(!in_array($book['type'],[1,2])){
                res_return('仅漫画和小说可生成推广链接');
            }
            $chapter = aSpread::getTenChapter($book_id);
            if(!$chapter){
                res_return('该书籍未添加章节');
            }
    ...
    public function doLink(){
        if($this->request->isAjax()){
            $field = 'id,book_id,name,is_sub,number,remark';
            aSpread::doneSpread($field);
        }else{
            $id = myRequest::getId('推广');
            $cur = aSpread::getById('Spread',$id);
            if(!$cur){
                res_return('推广链接参数异常');
            }
            $book = aSpread::getById('Book',$cur['book_id'],'id,name,type');
            if(!in_array($book['type'],[1,2])){
                res_return('仅漫画和小说可生成推广链接');
            }
            $chapter = aSpread::getTenChapter($cur['book_id']);
            if(!$chapter){
                res_return('该书籍未添加章节');
            }
            $book['chapter'] = $chapter;
    ...
    public function delLink(){
        ...
        $re = aSpread::setField('Spread',[['id','=',$id]],'status',2);
        if($re !== false){
            res_return('ok');
        }else{
            res_return('删除失败');
    ...
    public function copyLink(){
        $id = myRequest::getId('推广');
        $cur = aSpread::getById('Spread',$id,'id,url,short_url,status');
        if(empty($cur)){
            res_return('推广链接不存在');
        }
        if($cur['status'] != 1){
    ...
    public function qrcode(){
        $id = myRequest::getId('推广');
        $cur = aSpread::getById('Spread', $id,'id,url');
        if(!$cur){
            res_return('推广信息异常');
        }
        if(!$cur['url']){
