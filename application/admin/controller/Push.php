...
class Push extends Common{
    ...
    public function doPush(){
        if($this->request->isAjax()){
            $type = myValidate::getData(mPush::$rules, 'type');
            if($type != 1){
            ...
            $cur = mPush::getCur('TaskMessage',[['type','=',$type],['channel_id','=',0]],'id,status');
            if($cur){
                $re = mPush::save('TaskMessage', [['id','=',$cur['id']]], $data);
            }else{
                $data['status'] = 2;
                $re = mPush::add('TaskMessage', $data);
            }
            if($re){
                res_return('ok');
            }else{
            ...
            $type = myValidate::getData(mPush::$rules,'type','get');
            if($type != 1){
                res_return('该类型消息禁止编辑');
            }
            $data = mPush::getCur('TaskMessage',[['type','=',$type],['channel_id','=',0]]);
    ...
    public function doEvent(){
        $post = myValidate::getData(mPush::$rules, 'type,event');
        $res = mPush::donePushEvent($post);
        if($res){
            res_return('ok');
        }else{
            res_return('操作失败，请重试');
