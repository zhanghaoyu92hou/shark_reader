...
class mWx extends Common{
    ...
    public static function getMenuList(){
        $list = Db::name('WxMenu')->where('channel_id','=',0)->order('sort_num','desc')->select();
        return $list;
    }
    ...
    public static function getMenuCount($pid){
        $count = Db::name('WxMenu')->where('pid','=',$pid)->where('channel_id','=',0)->count();
        return $count;
    }
    ...
    public static function createDefaultMenu(){
    	...
    	$url = $website['is_location'] == 1 && $website['location_url'] ? $website['location_url'] : $website['url'];
    	$data = [
    		['name'=>'签到','channel_id'=>0,'pid'=>0,'type'=>3,'content'=>'{"appid": "", "value": "sign"}'],
    		['name'=>'进入书城','channel_id'=>0,'pid'=>0,'type'=>0,'content'=>'{"appid": "", "value": ""}','child'=>[
    				['name'=>'书城首页','channel_id'=>0,'pid'=>0,'type'=>1,'content'=>'{"appid": "", "value": "http://'.$url.'/index/Book/novel.html"}'],
    				['name'=>'个人中心','channel_id'=>0,'pid'=>0,'type'=>1,'content'=>'{"appid": "", "value": "http://'.$url.'/index/User/index.html"}'],
    				['name'=>'精品推荐','channel_id'=>0,'pid'=>0,'type'=>1,'content'=>'{"appid": "", "value": "http://'.$url.'/index/Book/more.html?type=2&is_hot=1"}'],
    				['name'=>'我要充值','channel_id'=>0,'pid'=>0,'type'=>1,'content'=>'{"appid": "", "value": "http://'.$url.'/index/Charge/index.html"}'],
    				['name'=>'联系客服','channel_id'=>0,'pid'=>0,'type'=>3,'content'=>'{"appid": "", "value": "lianxikefu"}'],
    		]],
    		['name'=>'阅读记录','channel_id'=>0,'pid'=>0,'type'=>1,'content'=>'{"appid": "", "value": "http://'.$url.'/index/User/myHistory.html"}'],
    	];
    	Db::startTrans();
    	$flag = false;
    	$re = Db::name('WxMenu')->where('channel_id','=',0)->delete();
    	if($re !== false){
    		foreach ($data as $v){
    			$child = '';
    			$result = false;
    ...
    public static function doneReply($field){
        ...
        $where = [['keyword','=',$data['keyword']],['channel_id','=',0]];
        if(isset($data['id'])){
            $where[] = ['id','<>',$data['id']];
        }
