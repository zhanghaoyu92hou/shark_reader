...
class cMember extends Common{
    ...
    public static function getMemberList($where,$pages){
        ...
        if($list){
            foreach ($list as &$v){
                ...
                $v['vip_str'] = $v['viptime'] > 0 ? '到期时间:'.date('Y-m-d',$v['viptime']) : 'N/A';
            }
    ...
    public static function getMemberCountMsg($uid){
        global $loginId;
        $chargeMoney = Db::name('Order')->where('uid','=',$uid)->where('channel_id','=',$loginId)->where('is_count','=',1)->where('status','=',2)->sum('money');
