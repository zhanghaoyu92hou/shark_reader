...
class cChart extends Common{
    ...
    public static function getMemberCount(){
        ...
        $today_list = Db::name('Member')->where('channel_id','=',$loginId)->where('create_time','>=',$cur_time)->field('id,subscribe,sex,is_charge')->select();
        if($today_list){
    ...
    public static function getOrderCount(){
        ...
        $today_list = Db::name('Order')->where('channel_id','=',$loginId)->where('is_count','=',1)->where('create_time','>=',$cur_time)->field('id,type,package,uid,status,money')->select();
        if($today_list){
            $n_uids = $p_uids = [];
            foreach($today_list as $v){
                if($v['status'] == 2){
